...
Router.register('clientes', async (c) => {
  ...
  function openForm(id) {
    ...
    document.getElementById('ms').onclick = async () => {
      ...
      const d = { id:cl?.id||Store.generateId(), nome:n, telefone:document.getElementById('f-t').value.trim(), endereco:document.getElementById('f-e').value.trim(), observacoes:document.getElementById('f-o').value.trim(), ativo:true, created_at:cl?.created_at||new Date().toISOString(), updated_at:new Date().toISOString() };
      cl ? await SyncEngine.update('clientes',d) : await SyncEngine.insert('clientes',d);
      Modal.close(); Toast.success(cl?'Atualizado!':'Cadastrado!'); Router.navigate('clientes');
    };
  }
  async function del(id) {
    ...
    document.getElementById('mc').onclick = () => Modal.close();
    document.getElementById('md').onclick = async () => { await SyncEngine.remove('clientes',id); Modal.close(); Toast.success('Excluído!'); Router.navigate('clientes'); };
  }
});
