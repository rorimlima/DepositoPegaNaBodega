...
Router.register('produtos', async (c) => {
  let produtos = await SyncEngine.getAll('produtos');
  ...
  function openForm(id) {
    ...
    document.getElementById('ms').onclick = async () => {
      ...
      const d = { id:p?.id||Store.generateId(), nome:n, categoria:document.getElementById('fp-c').value, preco_custo:parseFloat(document.getElementById('fp-pc').value)||0, preco_venda:parseFloat(document.getElementById('fp-pv').value)||0, estoque_atual:parseInt(document.getElementById('fp-e').value)||0, ativo:true, is_deleted:false, created_at:p?.created_at||new Date().toISOString(), updated_at:new Date().toISOString() };
      p ? await SyncEngine.update('produtos',d) : await SyncEngine.insert('produtos',d);
      Modal.close(); Toast.success(p?'Atualizado!':'Cadastrado!');
      // Optimistic update
      if (p) { Object.assign(p, d); } else { produtos.push(d); }
      render(document.getElementById('p-search')?.value || '');
    };
  }
  ...
  SyncEngine.subscribe(['produtos'], async () => {
    if (Router.current === 'produtos') {
      produtos = await SyncEngine.getAll('produtos');
      render(document.getElementById('p-search')?.value || '');
    }
  });
});
