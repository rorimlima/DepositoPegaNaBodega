...
// ─────────────────────────────────────────────────────────────────────────────
// Formulário FORA do componente pai — props via callback, não recria a cada render
// ─────────────────────────────────────────────────────────────────────────────
const ProdutoForm = memo(function ProdutoForm({ initial, onSave, onCancel }) {
  const [nome,       setNome]       = useState(initial?.nome       ?? '');
  const [codigo,     setCodigo]     = useState(initial?.codigo     ?? '');
  const [categoria,  setCategoria]  = useState(initial?.categoria  ?? '');
  const [preco,      setPreco]      = useState(initial?.preco      ?? '');
  const [custo,      setCusto]      = useState(initial?.custo      ?? '');
  const [quantidade, setQuantidade] = useState(initial?.quantidade ?? '');
  const [erro,       setErro]       = useState('');

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!nome || !preco) return setErro('Nome e Preço são obrigatórios.');
    setErro('');
    onSave({ nome, codigo, categoria, preco, custo, quantidade });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4 p-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input placeholder="0.00" type="number" step="0.01" min="0" value={custo}
            onChange={e => setCusto(e.target.value)} inputMode="decimal"
            className="bg-slate-900 border-slate-800 h-12" />
        </div>
        <div>
          <label className="text-xs text-slate-500 mb-1.5 block">Preço Venda *</label>
          <Input placeholder="0.00" type="number" step="0.01" min="0" value={preco}
            onChange={e => setPreco(e.target.value)} required inputMode="decimal"
            className="bg-slate-900 border-slate-800 h-12" />
        </div>
      </div>
      <div>
        <label className="text-xs text-slate-500 mb-1.5 block">Quantidade em Estoque</label>
        <Input placeholder="0" type="number" min="0" value={quantidade}
          onChange={e => setQuantidade(e.target.value)} inputMode="numeric"
          className="bg-slate-900 border-slate-800 h-12" />
      </div>
...
const ProdutoCard = memo(function ProdutoCard({ p, onEdit, onDelete }) {
  const margem = p.custo_centavos
    ? (((p.preco_centavos - p.custo_centavos) / p.custo_centavos) * 100).toFixed(0)
    : null;
    ...
    <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4">
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1 min-w-0 pr-2">
          ...
          {p.categoria && (
            <span className="inline-block mt-1 px-2 py-0.5 text-[10px] font-medium bg-blue-500/10 text-blue-400 rounded-full">
              {p.categoria}
            </span>
          )}
        </div>
        <div className="flex gap-1.5 shrink-0">
          <button onClick={() => onEdit(p)} className="w-9 h-9 rounded-xl bg-slate-800 flex items-center justify-center text-slate-400 active:bg-slate-700">
            <Edit size={13} />
          </button>
          <button onClick={() => onDelete(p.id)} className="w-9 h-9 rounded-xl bg-red-500/10 flex items-center justify-center text-red-500 active:bg-red-500/20">
            <Trash2 size={13} />
          </button>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2 pt-3 border-t border-slate-800/60">
        ...
        <div>
          ...
          <p className={`text-sm font-bold ${(p.quantidade || 0) > 5 ? 'text-green-400' : 'text-red-400'}`}>
            {p.quantidade || 0} un.
          </p>
        </div>
      </div>
      {margem && (
        <p className="text-[10px] text-slate-600 mt-2">
          Margem: <span className="text-green-400 font-semibold">{margem}%</span>
        </p>
      )}
    </div>
  );
...
export default function ProdutosPage() {
  ...
  const [editData,        setEditData]        = useState(null); // null = novo, objeto = editar
  ...
  const handleSave = useCallback(async ({ nome, codigo, categoria, preco, custo, quantidade }) => {
    const preco_centavos = Math.round(parseFloat(String(preco).replace(',', '.')) * 100);
    const custo_centavos = custo ? Math.round(parseFloat(String(custo).replace(',', '.')) * 100) : 0;
    const qtd = quantidade ? parseInt(String(quantidade), 10) : 0;

    if (editData?.id) {
      const updated = { id: editData.id, nome, codigo, categoria, preco_centavos, custo_centavos, quantidade: qtd };
  ...
  const handleEdit = useCallback((p) => {
    setEditData({
      ...
      preco:      ((p.preco_centavos || 0) / 100).toFixed(2),
      custo:      ((p.custo_centavos  || 0) / 100).toFixed(2),
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      ...
      <div className="flex flex-col lg:flex-row gap-5">
        {/* Desktop: form sempre visível na lateral */}
        <div className="hidden lg:block w-[340px] shrink-0">
          <div className="bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden">
            <div className="px-4 pt-4 pb-2 border-b border-slate-800">
              <h2 className="font-bold text-slate-100">{editData?.isEdit ? '✏️ Editar Produto' : '📦 Novo Produto'}</h2>
            </div>
            <ProdutoForm key={editData?.id ?? 'new'} initial={editData} onSave={handleSave} onCancel={editData ? handleCancel : undefined} />
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            <Select value={filtroCategoria} onValueChange={setFiltroCategoria}>
              <SelectTrigger className="bg-slate-900 border-slate-800 h-11 sm:w-[190px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {produtosFiltrados.map(p => (
              <ProdutoCard key={p.id} p={p} onEdit={handleEdit} onDelete={handleDelete} />
            ))}
        ...
        <div className="lg:hidden fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end">
          <div className="w-full bg-slate-950 border-t border-slate-800 rounded-t-3xl overflow-hidden relative"
            ...
            <div className="overflow-y-auto" style={{ maxHeight: 'calc(92dvh - 64px)' }}>
              <ProdutoForm key={editData?.id ?? 'new'} initial={editData} onSave={handleSave} onCancel={handleCancel} />
            </div>
          </div>
        </div>
      )}
