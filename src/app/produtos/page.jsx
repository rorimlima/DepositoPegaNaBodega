...
export default function ProdutosPage() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <h1 className="text-2xl font-bold text-blue-500">Gerenciar Produtos</h1>

      {/* ── KPI Cards ── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-slate-950 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-medium text-slate-400">Total Itens em Estoque</CardTitle>
            <Package className="h-4 w-4 text-purple-500" />
          </CardHeader>
          <CardContent><div className="text-xl font-bold text-slate-100">{totalItens.toLocaleString('pt-BR')}</div></CardContent>
        </Card>

        <Card className="bg-slate-950 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-medium text-slate-400">Custo do Estoque</CardTitle>
        ...
        <Card className="bg-slate-950 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-medium text-slate-400">Potencial de Venda</CardTitle>
        ...
        <Card className="bg-slate-950 border-slate-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-medium text-slate-400">Lucro Potencial</CardTitle>
            <DollarSign className="h-4 w-4 text-blue-500" />
          </CardHeader>
          <CardContent><div className="text-xl font-bold text-blue-500">R$ {((potencialVenda - custoEstoque) / 100).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div></CardContent>
        </Card>
      </div>
        ...
        <div className="w-full lg:w-[360px] shrink-0">
          <Card className="bg-slate-950 border-slate-800">
            <CardContent className="p-4 space-y-4">
              <h2 className="text-lg font-semibold text-slate-100">{editId ? '✏️ Editar Produto' : '📦 Novo Produto'}</h2>
              <form onSubmit={handleSubmit} className="space-y-3">
                <Input placeholder="Nome do Produto *" value={nome} onChange={e => setNome(e.target.value)} required className="bg-slate-900 border-slate-800" />
                <Input placeholder="Código (Ex: SK001)" value={codigo} onChange={e => setCodigo(e.target.value)} className="bg-slate-900 border-slate-800" />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">Custo (R$)</label>
                    <Input placeholder="0.00" type="number" step="0.01" min="0" value={custo} onChange={e => setCusto(e.target.value)} className="bg-slate-900 border-slate-800" />
                  </div>
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">Preço Venda (R$) *</label>
                    <Input placeholder="0.00" type="number" step="0.01" min="0" value={preco} onChange={e => setPreco(e.target.value)} required className="bg-slate-900 border-slate-800" />
                  </div>
                </div>

                <div>
                  <label className="text-xs text-slate-500 mb-1 block">Quantidade em Estoque</label>
                  <Input placeholder="0" type="number" min="0" value={quantidade} onChange={e => setQuantidade(e.target.value)} className="bg-slate-900 border-slate-800" />
                </div>
        ...
        <div className="flex-1">
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar por nome ou código..." value={busca} onChange={e => setBusca(e.target.value)} className="bg-slate-900 border-slate-800 pl-10" />
            </div>
            <Select value={filtroCategoria} onValueChange={setFiltroCategoria}>
              <SelectTrigger className="bg-slate-900 border-slate-800 w-full sm:w-[200px]">
